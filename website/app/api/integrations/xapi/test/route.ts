...
export async function POST(request: Request) {
  ...
  const { data: canManageIntegration, error: permissionError } =
    await supabase.rpc("has_permission", {
      target_permission: "integrations.manage",
      target_org_id: organizationId,
    });

  if (permissionError || canManageIntegration !== true) {
    return NextResponse.json(
      { message: "اختبار المفتاح متاح لمن يملك صلاحية إدارة التكاملات فقط." },
      { status: 403 },
    );
  }
