...
export type JotformWebhookAuthorization =
  | "authorized"
  | "misconfigured"
  | "rejected";

export function authorizeJotformWebhook(
  request: Request,
): JotformWebhookAuthorization {
  ...
  const suppliedSecret =
    request.headers.get("x-miqyas-webhook-secret") ??
    requestUrl.searchParams.get("token") ??
    "";

  if (suppliedSecret.length < 32 || suppliedSecret.length > 512) {
    return "rejected";
  }
