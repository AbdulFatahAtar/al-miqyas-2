...
export function TraineeDetailsPage({
  traineeCode,
  organizationId,
}: {
  traineeCode: string;
  ...
  const loadData = useCallback(async () => {
    ...
    if (enrollmentError) {
      setErrorMessage("تعذر تحميل تسجيل المتدرب داخل الجهة الحالية.");
      setIsLoading(false);
      return;
    }
    ...
    const queryFailure = [
      ...
      xapiEventsResult.error,
      impactReportResult.error,
      certificateResult.error,
    ].find(Boolean);

    if (queryFailure) {
      setErrorMessage(
        "تعذر تحميل سجل الأدلة كاملًا. لم تُحوّل البيانات الناقصة إلى حالة غير مكتملة.",
      );
      setIsLoading(false);
      return;
    }
    ...
    const { data: xapiEventRows } = xapiEventsResult;
    const { data: impactReport } = impactReportResult;
    ...
    setIsLoading(false);
  }, [organizationId, supabase, traineeCode]);
  ...
  useEffect(() => {
    ...
    void QRCode.toDataURL(url, {
      ...
      .catch(() => {
        if (isActive) {
          setQrError("تعذر إنشاء رمز الاستجابة السريعة لهذا الرابط.");
        }
      });
  ...
  const copyUrl = async () => {
    ...
    try {
      ...
      setCopied(false);
      setCopyError("تعذر نسخ الرابط تلقائيًا. حدده وانسخه يدويًا.");
    }
  };
      ...
      <section className="trainee-detail-grid">
        ...
        <aside className="content-section personal-link-card">
          ...
          {qrDataUrl && (
            <Image
              src={qrDataUrl}
              alt={`رمز الاستجابة السريعة ${data.trainee.code}`}
              width={192}
              height={192}
              unoptimized
            />
          ...
          >
            ...
            {data.xapiEvents.map((event) => (
              <div className="event-stream-row" role="row" key={event.id}>
                ...
                <span role="cell">{xapiVerbLabel(event.verb_id)}</span>
                <span role="cell">{xapiObjectLabel(event)}</span>
                <span className="xapi-event-result" role="cell">
