...
export function xapiObjectLabel(event: XapiDisplayRecord) {
  const object = nestedRecord(event.raw_statement, "object");
  const definition = object
    ? nestedRecord(object, "definition")
    : null;
  const name = definition ? nestedRecord(definition, "name") : null;
  const localizedName =
    name?.["ar-SA"] ?? name?.ar ?? name?.en ?? name?.["en-US"];
